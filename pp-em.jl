...
function training_circuit_generation_augmented_perturbation(layer, dt, J, h, angle_range::Float64=pi/20, U=0.0; sample_function = nothing, num_samples::Int = 10)
    ...
    if !(0.0 <= theta_h <= angle_range) && !(pi/2 - angle_range <= theta_h <= pi/2)
        change_theta_h = true
    else
        change_theta_h = false
    end
    if !(-angle_range <= theta_J <= 0.0) && !(-pi/2 <= theta_J <= -pi/2 + angle_range)
        change_theta_J = true
    else
        change_theta_J = false
    end
    if !(0.0 <= theta_U <= angle_range) && !(pi/2 - angle_range <= theta_U <= pi/2)
        change_theta_U = true
    else
        change_theta_U = false
    end
    
    function sample_theta_CPA(angle_range)
        # tht_h ∈ [0, angle_range] ∪ [π/2 - angle_range, π/2]
        tht_h = rand(Bool) ? rand(Uniform(0.0, angle_range)) : rand(Uniform(π/2 - angle_range, π/2))
    
        # tht_J ∈ [−angle_range, 0] ∪ [−π/2, −π/2 + angle_range]
        tht_J = rand(Bool) ? rand(Uniform(-angle_range, 0.0)) : rand(Uniform(-π/2, -π/2 + angle_range))

        # tht_U ∈ [0, angle_range] ∪ [π/2 - angle_range, π/2]
        tht_U = rand(Bool) ? rand(Uniform(0.0, angle_range)) : rand(Uniform(π/2 - angle_range, π/2))
    
        return tht_h, tht_J, tht_U
    end

    function sample_theta_small(angle_range)
        # tht_h ∈ [0, angle_range]
        tht_h =  rand(Uniform(0.0, angle_range)) 
    
        # tht_J ∈ [−angle_range, 0] ∪ [−π/2, −π/2 + angle_range]
        tht_J = rand(Uniform(-angle_range, 0.0))
        
        # tht_U ∈ [0, angle_range]
        tht_U =  rand(Uniform(0.0, angle_range)) 
    ...
    for _ in 1:num_samples
        if change_theta_h
            tht_h_perturbed, _, _ = sample_function(angle_range)
        else 
            tht_h_perturbed = theta_h
        end
        if change_theta_J
            _, tht_J_perturbed, _ = sample_function(angle_range)
        else 
            tht_J_perturbed = theta_J
        end
        if change_theta_U
            _, _, tht_U_perturbed = sample_function(angle_range)
        else 
...
function training_circuit_generation_strict_perturbation(layer, dt, J, h, angle_range::Float64 = pi/20, U=0.0; sample_function = nothing, num_samples::Int = 10)
    ...
    for _ in 1:num_samples
        tht_h_perturbed, tht_J_perturbed = sample_function(angle_range)
        training_thetas[theta_h_indices] .= tht_h_perturbed
        training_thetas[theta_J_indices] .= tht_J_perturbed
        push!(training_thetas_list, copy(training_thetas))
...
function training_trotter_time_evolution(nl, layer, observable, training_thetas::Vector{Vector{Float64}}; noise_kind="none", min_abs_coeff=0.0, max_weight = Inf, noise_level = 1.0, depol_strength=0.02, dephase_strength=0.02)
    ...
    for (i, thetas) in enumerate(training_thetas)
        exact_expvals[i, :] = trotter_time_evolution(
            nl, layer, observable, thetas;
...
function zne_time_evolution(nl, layer, observable, dt, J, h, U=0.0; noise_kind="noiseless", min_abs_coeff=0.0, max_weight = Inf, noise_levels = [1,1.5,2.0], depol_strength=0.02, dephase_strength=0.02)
    ...
    for (idx,i) in enumerate(noise_levels)
        expval_target = trotter_time_evolution(
            nl, layer, observable, thetas;
            noise_kind=noise_kind, noise_level=i,
            min_abs_coeff=min_abs_coeff, max_weight=max_weight,
            depol_strength=depol_strength, dephase_strength=dephase_strength)
...
function vnCDR_training_trotter_time_evolution(nl, layer, observable, training_thetas::Vector{Vector{Float64}}; noise_kind="none", min_abs_coeff=0.0, max_weight=Inf, noise_levels=[1, 1.5,2.0], depol_strength=0.02, dephase_strength=0.02)
    ...
    for (idx, i) in enumerate(noise_levels) # use enumerate to get valid index
        noisy_training = training_trotter_time_evolution(
            nl, layer, observable, training_thetas;
            noise_kind=noise_kind, noise_level=i,
            min_abs_coeff=min_abs_coeff, max_weight=max_weight,
            depol_strength=depol_strength, dephase_strength=dephase_strength)
...
function full_run_all_methods(nq, nl, topology, layer, J, h, dt,
    ...
    """
    U = 0.0
    @logmsg SubInfo "→ Starting full_run_all_methods (noise_kind=$noise_kind, σ=$angle_range)"
    T = dt*nl
    ...
    training_set = training_circuit_generation_strict_perturbation(layer, dt, J, h, angle_range, U; num_samples=num_samples)
    end
    ...
    if use_target
        thetas = define_thetas(layer, dt, J, h)
        exact_target = trotter_time_evolution(nl, layer, observable, thetas;
                        noise_kind="none",
                        min_abs_coeff=min_abs_coeff_target)
        timetmp = time()
        @logmsg SubInfo "→ exact_target done in $(round(timetmp - time1; digits=2)) s"

        noisy_target = trotter_time_evolution(nl, layer, observable, thetas;
    ...
    exact_train = training_trotter_time_evolution(nl, layer, observable, training_set;
                        noise_kind="none",
    ...
    noisy_train = training_trotter_time_evolution(nl, layer, observable, training_set;
                        noise_kind=noise_kind,
    ...
    noisy_train = noisy_train[:, end]

    timetmp2 = time()
    @logmsg SubInfo "→ noisy_train done in $(round(timetmp2 - timetmp1; digits=2)) s"

    # --- ZNE ---
    zne_levels = zne_time_evolution(nl, layer, observable,dt, J, h, U;
            noise_kind=noise_kind,
            min_abs_coeff=min_abs_coeff,
            max_weight=max_weight,
    ...
    if record_fit_data
        ...
        fn = format("ZNE_noise_{}_hval_{:.3e}_nq={:d}_nl={:d}_angledef={:.3e}.dat",noise_kind, h*2*dt, nq, nl,angle_range)
        mkpath(dirname(fn))
    ...
    open(tmp,"a") do log
        ...
        "CDR", angle_range, nl,T, J, h, obs_string, nq,
    ...
    # code to plot the CDR fits
    # scatter(noisy_train, exact_train, label="noisy vs exact", xlabel="noisy expectation value", ylabel="exact expectation value")
    # scatter!([real_qc_noisy_data], [result_cdr], label="CDR result", color=:red)
    # plot!(xlabel="noisy expectation value", ylabel="exact expectation value")
    # savefig("jl_CDR_fit_h_$(h).png")

    # --- vnCDR ---
    noisy_train_multi = vnCDR_training_trotter_time_evolution(nl, layer, observable, training_set;
                                    noise_kind=noise_kind,
                                    min_abs_coeff=min_abs_coeff,
                                    max_weight=max_weight,
                                    noise_levels=noise_levels,
    ...
    open(tmp,"a") do log
        str = format(
        "{:>10s} {:>10.2e}{:>3n}{:>6.2e}{:>10.2e}{:>10.2e} {:>2s} {:>5n} {:>10.3e} {:>10.3e} {:>10.3e}  {:>10.3e} {:>10.3e} {:>8.2f}\n",
        "vnCDR",angle_range, nl,T, J, h, obs_string, nq,
